...
class ShowSimilarity(object):
	...
	def __load_data(self):
		self.__show_data = pickle.load(open('F:/imdb/shows_data.pickle', 'rb'))
		print('Loaded book data.')
		cv = CountVectorizer()
		count_matrix = cv.fit_transform(self.__show_data['soup'])
		self.__cos_sim = cosine_similarity(count_matrix)
	...
	if searchText:
		...
		if results is not None:
			output = results[['title', 'year', 'language', 'genre', 'rating',\
			                  	'stars', 'creators', 'episode_season_duration', 'trailer', 'nameurls']].to_dict(orient='records')#results.title.values
